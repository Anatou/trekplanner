...
func _process(delta: float) -> void:
	...
	if Input.is_action_pressed(&"map_left"):   map.move_relative_to_gps(0, -delta*0.01)
	if Input.is_action_pressed(&"map_right"):  map.move_relative_to_gps(0, delta*0.01)
	if Input.is_action_pressed(&"map_top"):    map.move_relative_to_gps(delta*0.01, 0)
	if Input.is_action_pressed(&"map_bottom"): map.move_relative_to_gps(-delta*0.01, 0)
